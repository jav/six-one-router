...
u_int start_sixone(sixone_settings settings)
{
  ...
  // (redundant) add_route(&default_route, 0, "tun0", global_settings);
  ...
  for( i=0; i < global_settings->if_c; i++)
	...
	if(!global_settings->if_v[i]->net_v[j]->edge) {
	  inet_ntop(AF_INET6, &global_settings->if_v[i]->net_v[j]->addr->ip, ip_str, sizeof(ip_str));
	  
	  printf("Adding route: %s/%p on if: %s : returned %d\n", 
		 ip_str, 
		 global_settings->if_v[i]->net_v[j]->addr->pfx,
		 global_settings->if_v[i]->if_name,
		 add_route(&global_settings->if_v[i]->net_v[j]->addr->ip, 
			   global_settings->if_v[i]->net_v[j]->addr->pfx,
      ...
      */
      ...
      //DBG_P(" global_settings: %p\n", global_settings);
      set_n_if = (u_char**) malloc(sizeof(sixone_settings) + sizeof(sixone_if));
      set_n_if[0] = (u_char*)global_settings;
      //DBG_P(" set_n_if[0] = global_settings;\n");
      set_n_if[1] = (u_char*)global_settings->if_v[i];
      //DBG_P(" : set_n_if[0]:%p, set_n_if[1]:%p (%s)\n",set_n_if[0], set_n_if[1], ((sixone_if)set_n_if[1])->if_name);
...
void* start_interface(void* args)
{
  ...
  //DBG_P("%s:%d : set_n_if[0]:%p, set_n_if[1]:%p\n", set_n_if[0], set_n_if[1]);
  //DBG_P("malloc(%d) for %s\n", strlen(_dev->if_name) + 1, _dev->if_name);

  //DBG_P("starting: %s\n", _dev->if_name );
  // setup the device
      
  //DBG_P("\tpcap_open_live(%s, %d, %d, %d, sixone_errbuff)\n", _dev->if_name, BUFSIZ, 0, 1000);
...
int set_filter(pcap_t *handle, sixone_if dev)
{
  ...
  for( i=0; i < global_settings->if_c; ++i) {
	...
	//DBG_P(" : set_filter() - adding strlen (a) : %u\n", __FILE__, __LINE__, _strlen+1);
	//DBG_P(" : set_filter() - global_settings->if_c : %u \n", __FILE__, __LINE__, global_settings->if_c);
	for(k=0;k < global_settings->if_c; ++k) {
	  _strlen += strlen(" and not ip6 src ") + INET6_ADDRSTRLEN;
	  //DBG_P(" : set_filter() - adding strlen (b): %u\n", __FILE__, __LINE__, _strlen+1);
	}
	_strlen += strlen( " )");
      }
  ...
  //DBG_P(" : set_filter() - malloc() ing %d bytes\n", __FILE__, __LINE__, _strlen+1);
  ...
  for( i=0; i < global_settings->if_c; ++i) {
    ...
    if(j > 0) {
      ...
      //DBG_P(" : set_filter() - str_net is: %s\n", __FILE__, __LINE__, str_net);
...
void inbound(struct ip6_hdr *ip)
{
  ...
  // add route to edgenet
  // DBG_P(" : inbound() : add route\n", __FILE__, __LINE__);
  // (redundant) add_route( &ip->ip6_dst, 128,"tun0", global_settings);
  
  // resolve to source edge
  //if( is_sixone( ((sixone_ip){&ip->ip6_src, 128}) ) ) {
  if( bilateral_bit(ip) ) {
    ...
    //#ifdef CKSUMNEUTRALIP
    //cksumNeutralIp( &ip->ip6_dst, &old->ip);
    /*#else
    switch(ip->ip6_nxt) {
    case IPPROTO_IP:
    case IPPROTO_TCP:
      recalc_tcp_checksum(ip);
      break;
    case IPPROTO_UDP:
      recalc_udp_checksum(ip);
      break;
    case IPPROTO_ICMP:
      recalc_icmp6_checksum(ip);
      break;
    }
...
void outbound(struct ip6_hdr *ip)
{
  ...
  if( is_sixone((sixone_ip){&ip->ip6_dst , 128}) ) {
    ...
    // add route to edgenet
    // DBG_P(" : outbound() : add route\n", __FILE__, __LINE__);
    // (redundant) add_route( &ip->ip6_dst, 128,"tun0");
...
void forward_packet(struct ip6_hdr *ip)
{
  ...
  if((sizeof(*ip) + ip->ip6_plen) > 15000) {
    ...
    // Send an ICMP - Fragmentation Needed packet.
    return;
  }
  ...
  //DBG_P(" : forward_packet( ) : family is %hd\n", __FILE__, __LINE__, AF_INET6);
...
void extract_prefix(u_char* buffer, u_int offset, u_int totLen)
{
  ...
  if( 0 == offset % 8 ) {
    //DBG_P("%s:%d : extract_prefix(%p, %d, %d) - %d, %d, %d\n", buffer, offset, totLen, offset/8, totLen/8, totLen/8 - offset/8);
    //DBG_P("%s:%d : extract_prefix(%p, %d, %d) - %p, %p, %p\n", buffer, offset, totLen, offset/8, buffer, &buffer[offset/8], &buffer[offset/8 + (totLen/8 - offset/8)]);
    memset(&buffer[offset/8], 0, totLen/8 - (offset/8));
    //DBG_P("%s:%d : extract_prefix(%p, %d, %d)\n", buffer, offset, totLen);
  }
  else {
...
u_int is_inbound(struct ip6_hdr *ip)
{
  ...
  //DBG_P("%s:%d : is_inbound() : returning src:%d, dst:%d \n", src, dst);
  return 0;
}
...
u_int is_outbound(struct ip6_hdr *ip)
{
  ...
  //DBG_P("%s:%d : is_outbound() : returning src:%d, dst:%d \n", src, dst);
  return !(dst) && src;
}
